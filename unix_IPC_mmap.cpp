...
int main (int argc, char* argv[])
{
  ...
  double *shared_space = (double*)mmap(NULL, shared_space_bytes, PROT_READ | PROT_WRITE, MAP_ANONYMOUS | MAP_SHARED, -1, 0);
...
/*
    ...
    for (int proc = 1 ; proc <= NUM_PROCS ; proc++) {
      double *recv_buf = shared_space + (proc - 1) * shared_buf_size_bytes / sizeof(double);
...
*/
  ...
  munmap(shared_space, shared_space_bytes);
  return 0;
}
